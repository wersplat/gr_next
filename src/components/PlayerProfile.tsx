...
import type { PlayerWithTeam, Team } from '@/types/player';
...
// Extend the PlayerWithTeam type to include additional properties used in the component
interface ExtendedPlayerWithTeam extends Omit<PlayerWithTeam, 'teams'> {
  player_rank_score?: number;
  player_rp?: number;
  performance_score?: number;
  teams?: Team | Team[] | null;  // Make teams more flexible to handle both array and single object
}

const PlayerProfile: React.FC<PlayerProfileProps> = ({ player: playerProp }: PlayerProfileProps) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('md'));
  const [tabValue, setTabValue] = useState(0);
  
  // Safely cast player to ExtendedPlayerWithTeam
  const player = playerProp as ExtendedPlayerWithTeam;
            ...
            <Box sx={{ textAlign: { xs: 'center', md: 'left' } }}>
              ...
              {player.teams && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  <Avatar 
                    src={Array.isArray(player.teams) ? player.teams[0]?.logo_url || undefined : player.teams.logo_url || undefined} 
                    alt={Array.isArray(player.teams) ? player.teams[0]?.name : player.teams.name}
                    sx={{ width: 24, height: 24 }}
                  />
                  <Link href={`/teams/${Array.isArray(player.teams) ? player.teams[0]?.id : player.teams.id}`} style={{ textDecoration: 'none' }}>
                    <Typography 
                      variant="subtitle1" 
                      color="primary"
                      sx={{ '&:hover': { textDecoration: 'underline' } }}
                    >
                      {Array.isArray(player.teams) ? player.teams[0]?.name : player.teams.name}
                    </Typography>
                  </Link>
                </Box>
              )}
                      ...
                      <TableBody>
                        ...
                        <TableRow>
                          ...
                          <TableCell align="right">
                            {player.teams ? (
                              <Link 
                                href={`/teams/${Array.isArray(player.teams) ? player.teams[0]?.id : player.teams.id}`} 
